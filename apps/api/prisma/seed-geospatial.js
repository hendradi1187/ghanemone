...
const STATUS_CYCLE = ['public', 'internal', 'internal', 'confidential'];
const LICENSES = {
    public: 'CC-BY-4.0',
    internal: 'Internal SPEKTRUM',
    confidential: 'Restricted SKK Migas',
};
function sensitivityFromStatus(s) {
    if (s === 'public')
        return 'PUBLIC';
    if (s === 'confidential')
        return 'CONFIDENTIAL';
...
function descriptionFor(cat, region, provider) {
    switch (cat) {
...
function buildDatasets() {
    ...
    for (const catDef of CATEGORY_DEFS) {
        for (let j = 0; j < catDef.perCat; j += 1) {
            ...
            const accuracyValues = ['high', 'high', 'medium', 'high', 'low', 'high', 'medium'];
            // Index mathematically in bounds via modulo — safe to assert non-null
...
async function seedWorkAreas() {
    ...
    `;
        ...
        if (!id)
            throw new Error(`Failed to upsert work area: ${wk.slug}`);
...
async function seedDatasets(wkSlugToId) {
    ...
    for (const ds of datasets) {
        ...
        const workAreaId = CONCESSION_WK_MAP[ds.id] != null
...
async function seedWells(wkSlugToId) {
...
async function seedFacilities(wkSlugToId) {
...
async function seedPipelines(wkSlugToId) {
...
async function seedSeismicCoverages(wkSlugToId) {
...
// Utility
// =============================================================================
function computeBboxFromPolygon(ring) {
...
async function verifySeeding() {
    console.log('\n--- Verification ---');
    const waCount = await prisma.$queryRaw `
    SELECT
      COUNT(*)::text AS count,
  ...
  `;
    ...
    // Verify SRID
    const sridCheck = await prisma.$queryRaw `
...
//# sourceMappingURL=seed-geospatial.js.map